...
public class PlayerHomeScreen implements Screen {
	...
	public PlayerHomeScreen(MyGdxGame game) {
		this.game = game;
	}
	...
	private void update(float delta) {
		...
		if(startTimer >= 2 && !soundStarted){
			...
			showBubble = true;
			soundStarted = true;
			currentTextline = "Endlich bist du Zuhause,\n" + IOController.getProfile1Name() + "!";
		}
		...
		if(startTimer >= 11.5){
			currentTextline = "Packe bitte noch deine\nSchultasche, bevor du\nschlafen gehst!";
		}
		
		else if(startTimer >= 8){
			currentTextline = "Du hast doch morgen deinen\nersten Schultag!";
		}
		
		else if(startTimer >= 5){
			currentTextline = "Du bist spät dran.";
		}
